...
static void LCD_FSMCConfig(void)
{
  ...
  /* FSMC���ٶ����� */
  FSMC_NORSRAMTimingInitStructure.FSMC_AddressSetupTime = 10;  /* ��ַ����ʱ��  */
  FSMC_NORSRAMTimingInitStructure.FSMC_AddressHoldTime = 0;	   
  FSMC_NORSRAMTimingInitStructure.FSMC_DataSetupTime = 10;	   /* ��ݽ���ʱ��  */
  ...
  /* FSMCд�ٶ����� */
  FSMC_NORSRAMTimingInitStructure.FSMC_AddressSetupTime = 3;   /* ��ַ����ʱ��  */
  FSMC_NORSRAMTimingInitStructure.FSMC_AddressHoldTime = 0;	   
  FSMC_NORSRAMTimingInitStructure.FSMC_DataSetupTime = 3;	   /* ��ݽ���ʱ��  */
  FSMC_NORSRAMTimingInitStructure.FSMC_BusTurnAroundDuration = 0x00;
  FSMC_NORSRAMTimingInitStructure.FSMC_CLKDivision = 0x00;
  FSMC_NORSRAMTimingInitStructure.FSMC_DataLatency = 0x00;
  FSMC_NORSRAMTimingInitStructure.FSMC_AccessMode = FSMC_AccessMode_A;	/* FSMC ����ģʽ */
...
/******************************************************************************
* Function Name  : LCD_AddressSet
* Description    : Set the display RAM window
* Input          : x1
* 				 : y1
* 				 : x2
* 				 : y2
...
//      LCD_WriteReg(0x0021, Ypos );
 ...
 case SSD1289:   /* 0x8989 */
	 ...
	 LCD_WriteReg(0x44, (x2 << 8) + x1);
	 LCD_WriteReg(0x45, y1);
	 LCD_WriteReg(0x46, y2);
	 LCD_WriteReg(0x4e, x1);
	 LCD_WriteReg(0x4f, y1);
 ...
 case HX8347D: 	 /* 0x0047 */
	 ...
	 LCD_WriteReg(0x02, y1 >> 8); // Column address start2
	 LCD_WriteReg(0x03, y1); // Column address start1
	 LCD_WriteReg(0x04, y2 >> 8); // Column address end2
	 LCD_WriteReg(0x05, y2); // Column address end1
	 LCD_WriteReg(0x06, x1 >> 8); // Row address start2
	 LCD_WriteReg(0x07, x1); // Row address start1
	 LCD_WriteReg(0x08, x2 >> 8); // Row address end2
	 LCD_WriteReg(0x09, x2); // Row address end1
...
/******************************************************************************
* Function Name  : LCD_GetDisplayType
* Description    : Return code of detected TFT display
* Input          : None
* Output         : ID-code of Display
* Return         : None
* Attention		 : None
*******************************************************************************/
uint8_t LCD_GetDisplayType (void) {
	return LCD_Code;
}
...
* Description    : Fills a rectangle of the screen
* Input          : x1
* 				 : y1
* 				 : x2
* 				 : y2
...
void LCD_FillRect(uint16_t x1, uint16_t y1, uint16_t x2, uint16_t y2, uint16_t color) {
	...
	LCD_AddressSet(0, 0, MAX_X-1, MAX_Y-1);
}
